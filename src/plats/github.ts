...
export default <PlatformViaBrowser> {
	...
	async performLogin(page: Page): Promise<Result> {
		...
		if (!username || !password) {
			return Err("Could not find the GitHub username or password. Please check out your environment file.");
		}
		...
		if (await page.locator("#js-flash-container div.flash-error").isVisible()) {
			return Err("Wrong GitHub username or password. Please check out your environment file.");
		...
		return Err("Unexpected error occurred while trying to log into GitHub.");
	},

	async performVerify(page: Page): Promise<Result> {
		if (!await page.locator("#app_totp").isVisible()) {
			return Err("Expected to find the TOTP input, but it was not found while trying to verify in GitHub.");
		}

		const secret = this.credentials.secret;
		if (!secret) {
			return Err("Could not find the GitHub 2FA secret. Please check out your environment file.");
		}
		...
		if (await page.locator("#js-flash-container div.flash-error").isVisible()) {
			return Err("Wrong GitHub TOTP. Please check out your GitHub 2FA secret in your environment file.");
		} else if (await page.locator("body.logged-in").isVisible()) {
			return Ok();
		}

		return Err("Unexpected error occurred while trying to verify in GitHub.");
	},
	...
	async performUpdate(page: Page, image: string): Promise<Result> {
		...
		try {
			...
			return Err("Unexpected error occurred while trying to update your pfp in GitHub.");
		}
	},
};
