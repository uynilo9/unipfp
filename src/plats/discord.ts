...
export default <PlatformViaBrowser> {
	...
	async performLogin(page: Page): Promise<Result> {
		...
		if (!email || !password) {
			return Err("Could not find the Discord email or password. Please check out your environment file.");
		}
		...
		if (await page.locator("div[class*=helperTextContainer]").first().isVisible()) {
			switch (await page.locator("div[class*=helperTextContainer]").count()) {
				case 1: {
					return Err("New Discord account login location detected. Please check out your email inbox.");
				}
				case 2: {
					return Err("Wrong Discord email or password. Please check out your environment file.");
				}
				default: {
					return Err("Unexpected error occurred while trying to log into Discord.");
				}
			}
	...
	async performVerify(page: Page): Promise<Result> {
		if (!await page.locator("input[autocomplete=one-time-code]").isVisible()) {
			return Err("Expected to find the TOTP input, but it was not found while trying to verify in Discord.");
		}

		const secret = this.credentials.secret;
		if (!secret) {
			return Err("Could not find the Discord 2FA secret. Please check out your environment file.");
		}
		...
		if (await page.locator("div[class*=error]").isVisible()) {
			return Err("Wrong Discord TOTP. Please check out your Discord 2FA secret in your environment file.");
		} else if (await page.locator("nav[class*=guilds]").isVisible()) {
			return Ok();
		}

		return Err("Unexpected error occurred while trying to verify in Discord.");
	},
	...
	async performUpdate(page: Page, image: string): Promise<Result> {
		...
		try {
			...
			return Err("Unexpected error occurred while trying to update your pfp in Discord.");
		}
	},
};
