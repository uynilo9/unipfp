...
export default <PlatformViaBrowser> {
	...
	async performLogin(page: Page): Promise<Result> {
		...
		if (!username || !password) {
			return Err("Could not find the Twitter(X) username or password. Please check out your environment file.");
		}
		...
		if (await page.locator("div[data-testid=toast]").isVisible()) {
			return Err("Wrong Twitter(X) username. Please check out your environment file.");
		}
		...
		if (await page.locator("div[data-testid=toast]").isVisible()) {
			return Err("Wrong Twitter(X) password. Please check out your environment file.");
		...
		return Err("Unexpected error occurred while trying to log into Twitter(X).");
	},

	async performVerify(page: Page): Promise<Result> {
		if (!await page.locator("input[data-testid=ocfEnterTextTextInput]").isVisible()) {
			return Err("Expected to find the TOTP input, but it was not found while trying to verify in Twitter(X).");
		}

		const secret = this.credentials.secret;
		if (!secret) {
			return Err("Could not find the Twitter(X) 2FA secret. Please check out your environment file.");
		}
		...
		if (await page.locator("div[data-testid=toast]").isVisible()) {
			return Err("Wrong Twitter(X) TOTP. Please check out your Twitter(X) 2FA secret in your environment file.");
		} else if (await page.locator("button[data-testid=SideNav_AccountSwitcher_Button]").isVisible()) {
			return Ok();
		}

		return Err("Unexpected error occurred while trying to verify in Twitter(X).");
	},
	...
	async performUpdate(page: Page, image: string): Promise<Result> {
		...
		try {
			...
			return Err("Unexpected error occurred while trying to update your pfp in Twitter(X).");
		}
	},
};
